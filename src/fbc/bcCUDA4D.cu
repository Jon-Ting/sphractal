...
__global__ void BCKernel4D(unsigned char* M, const long long int m, const unsigned long long int bits_m, const unsigned long long int sm,
						   const unsigned long long bits_s, const unsigned long long bits_TPB, unsigned int* n)
{
	...
	// identifies grid index (i, j, k, l) from block and thread values
	...
	// global location of position (0, 0, 0, 0) of the grid (i, j, k, l) is gl + gk + gi + gj
	...
	const register unsigned long long int gl = (((l << bits_s) << bits_m) << bits_m) << bits_m;
	...
	const register unsigned long long int glsm = ((((l << bits_s) + sm) << bits_m) << bits_m) << bits_m;

	// compute and store the occupancy value of the grid (i, j, k, l)
	M[gl + gk + gi + gj] = M[gl + gk + gi + gj]     || M[gl + gk + gi + gjsm]     || M[gl + gk + gism + gj]     || M[gl + gk + gism + gjsm]   ||
					       M[gl + gksm + gi + gj]   || M[gl + gksm + gi + gjsm]   || M[gl + gksm + gism + gj]   || M[gl + gksm + gism + gjsm] ||
		                   M[glsm + gk + gi + gj]   || M[glsm + gk + gi + gjsm]   || M[glsm + gk + gism + gj]   || M[glsm + gk + gism + gjsm] ||
		                   M[glsm + gksm + gi + gj] || M[glsm + gksm + gi + gjsm] || M[glsm + gksm + gism + gj] || M[glsm + gksm + gism + gjsm];
	atomicAdd(n, M[gl + gk + gi + gj]);
}

cudaError_t CudaBC4D(const unsigned char* M, const long long int m, const unsigned char bits_m, const unsigned int TPB, const int nn, unsigned int* n)
{
	...
	cudaStatus = cudaMalloc((void**)&device_M, m * m * m * m * sizeof(unsigned char));
	if (cudaStatus != cudaSuccess) {
	...
	cudaStatus = cudaMemcpy(device_M, M, m * m * m * m * sizeof(unsigned char), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	while (size > 2) {
		...
		unsigned long long int num_box = (m * m * m * m) / (s * s * s * s);

		if (num_box >= TPB) {
			grid.x = ceilf(num_box / (float)TPB); // m/s * m/s * m/s * m/s= (grid_size * TPB)
			grid.y = 1; grid.z = 1;
		}
		else {
		...
		BCKernel4D << <grid, block >> > (device_M, m, bits_m, sm, ni + 1, bits_TPB, &device_n[ni]);
